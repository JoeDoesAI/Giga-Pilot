...
class FileUploader:
    ...
    changes the filename and stores it = create a unique file name
        input - UploadFile, database + model
        output - none
    ...
    async def validate_file(self, file: UploadFile) -> dict:
        ...
        if file_ext not in self.allowed_extensions:
            return {
                "valid": False,
                "error": f"The file extension {file_ext} not allowed",
            }
    ...
    async def save_file_name(self, filename: str, unique_filename: str):
        # Delegate database persistence to centralized CRUD functions
        from database.postgre.crud import create_file_record

        await create_file_record(
            self.database, original_name=filename, stored_name=unique_filename
        )
    ...
    async def upload_file(self, file: UploadFile, file_path: str) -> dict:
        try:
            ...
            await self.supabase_client.storage.from_(self.supabase_bucket).upload(
                ...
                file=content,
                file_options={
                    "content-type": file.content_type,  # Keeps it as PDF/Image/etc.
            ...
            return {
                "uploaded": False,
                "error": f"file was not uploaded due to {repr(e)}",
            }
